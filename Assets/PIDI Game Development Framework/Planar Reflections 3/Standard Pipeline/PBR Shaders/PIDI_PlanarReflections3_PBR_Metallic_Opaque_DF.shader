 ...
 *  
*/
Shader "PIDI Shaders Collection/Planar Reflections 3/PBR/Metallic (Depth Fade)" {
	...
	Properties
	{
		...
		[NoScaleOffset]_MetallicGlossMap("Metallic (R) Gloss(A)", 2D) = "gray" {}
		...
		SubShader{
			...
			void surf(Input IN, inout SurfaceOutputStandard o) {
				...
				float rDepth = tex2D(_ReflectionDepth, screenUV).g;
				
				half fDepth = 1-saturate(pow(pow(rDepth,2),_DepthFadeLevel*2));

				rDepth = pow(rDepth, 32);
