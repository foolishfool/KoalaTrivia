 ...
 *  
*/
Shader "PIDI Shaders Collection/Planar Reflections 3/PBR/Roughness" {
	...
	Properties
	{
		...
		[Header(Glossiness)]
		[Space(10)]
		[Enum(Value,0,Metallic Alpha,1)] _RghSource("Roughness Source", Float) = 0
		_Glossiness("Roughness", Range(0.0, 1.0)) = 0.5
		[NoScaleOffset]_SpecGlossMap("Gloss Map (R)", 2D) = "black" {}
		...
		SubShader{
			...
			void surf(Input IN, inout SurfaceOutputStandard o) {
				...
				fixed roughness = met.a;

				roughness = lerp(1 - _Glossiness, 1 - roughness, _RghSource);
				...
				half blur = saturate( saturate(1.1 - roughness) - lerp( 0, 1-rDepth, _ReflectionDepthInfluence ) );
