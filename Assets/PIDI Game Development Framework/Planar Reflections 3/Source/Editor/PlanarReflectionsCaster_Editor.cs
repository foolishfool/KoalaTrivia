...
namespace PlanarReflections3 {
    ...
    public class PlanarReflectionsCaster_Editor : Editor {
        ...
        public override void OnInspectorGUI() {
            ...
            Undo.RecordObject( refCaster, "" + refCaster.GetType() + refCaster.GetInstanceID() );
            ...
            if (BeginCenteredGroup("GENERAL SETTINGS", ref refCaster.folds[0])) {
                ...
                for (int i = 0; i < refCaster.castReflection.Length; i++) {
                    ...
                    refCaster.castDepth[i] = EnableDisableToggle(new GUIContent("Reflection Depth", "Enables the reflection depth texture for this material"), refCaster.castDepth[i]);

                    if ( refCaster.BlurMaterial ) {
                        GUILayout.Space( 8 );
                        refCaster.blurSettings[i].useBlur = EnableDisableToggle( new GUIContent( "Use Blur", "Whether the reflection for this material should be blurred" ), refCaster.blurSettings[i].useBlur );
                        
                        if ( refCaster.blurSettings[i].useBlur ) {
                            refCaster.blurSettings[i].blurPassMode = PopupField( new GUIContent( "Blur Pass Mode", "In which mode will the blur be applied to the reflection, either to the final target or as a separate pass (if it is applied as a separate pass, the shader itself must implement the _BlurReflectionTex parameter" ), refCaster.blurSettings[i].blurPassMode, new string[] { "Final Result", "Separate Pass" } );
                            refCaster.blurSettings[i].blurDownscale = IntSliderField( new GUIContent( "Blur Pass Downscale" ), refCaster.blurSettings[i].blurDownscale, 1, 4 );
                            refCaster.blurSettings[i].blurRadius = SliderField( new GUIContent( "Max. Blur Radius" ), refCaster.blurSettings[i].blurRadius );
                        }
        ...
        public void AssetLogoAndVersion() {
            ...
            GUILayout.Label("v" + refCaster.Version, pidiSkin2.customStyles[2]);
            GUILayout.Space(6);
            GUILayout.EndHorizontal();
            GUILayout.EndVertical();
        }
